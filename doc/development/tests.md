...
- `#disputable` (`input_shortcuts_click_spec`) — marks a contract whose *desirability* is contested even though the assertion is factually true. One group remains: global shortcuts fire without consuming the key. The second group — the console line receiving what a hidden widget declined — was retired on 2026-08-03 when the console route lost its widget step entirely (`../decisions/input.md`, Decision 1); those rows moved to the project route in `input_widget_lifecycle_spec`, where a hidden widget is a real decision, and the tag went with the dispute.
