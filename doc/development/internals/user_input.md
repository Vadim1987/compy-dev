...
Global shortcuts intercepted in `love.handlers.keypressed` (`controller.lua:778+`) before anything reaches the active route's controller: Ctrl+Pause suspends, Ctrl+Q quits project, Ctrl+S stops a run, Ctrl+Shift+R resets application, Ctrl+Alt+R restarts project, Ctrl+T quick-switches between run and edit. Ctrl+Esc (quit) is the one exception living on the release side — `love.handlers.keyreleased` (`controller.lua:889+`), not keypressed.

Each of these matches its modifier set **exactly** — the modifiers it names held, and no other (`decisions/input.md`, Decision 33). Ctrl+S in the **editor** is not on this list and never reaches it: closing a buffer and finishing an edit are `EditorController`'s own, decided inside the route (`editorController.lua`, `_save_keys`). None of these consume the key: it still reaches the active route afterward (§6.3 non-consuming shortcuts).
