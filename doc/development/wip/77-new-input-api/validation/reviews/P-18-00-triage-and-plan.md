...
| step | commit(s) | note |
|---|---|---|
| **P-18-07** D1, the menu digit | `4d2c881` + platform `74df3533` | `spendGlyph(k)` before `gotoScene(id)`. **Measured, fault and fix**, under real LÖVE against the real `menu.lua`/`scene.lua`/`input.lua` with the platform surface stubbed: keypressed-first order judged `"5"` before and nothing after; textinput-first was harmless before and stays so. Smoke row `A4`; the design of record gains the general rule (*whoever consumes a key owns it*) |
| **P-18-08** D2, `Alt+Shift+P` | `8a7b120` + platform `6545d633` | `alt+p` and `alt+shift+p` share one handler value, as the other four members of the family already do. **Measured before and after** by registering through `inputInit` and invoking the entries: before, `alt+shift+p` was NOT REGISTERED; after, both pause once on a fresh press and not on the repeat, and both claim `p`. The comment that hid it (*"each gesture is therefore bound twice"*, false of `alt+p` alone) is corrected in the same commit. Smoke row `D8b` |
| **P-18-11** O4, the owner's `REMARK:` | `4983454` | **Resolved, not swept.** The `setTextInput(true)` it questions is **upstream's own line**; text input is already on where this is developed (**measured**: `hasTextInput()` true at boot, `false`/`true` round-trips); and the IDE makes the same call at boot under a block its authors label *"Android specific settings"* (`src/main.lua:296`). So the line is what makes the game correct as a standalone LÖVE program on a device, and is redundant inside the IDE — which also settles why undoing it on exit is a no-op. The header's *"to match the IDE default"* named the right effect and the wrong reason and is rewritten. **The Android half rests on the framework's comment, not on a measurement** — nothing here can run an `.apk`. `grep -rn 'REMARK:\|INTERIM:'` is now empty in the example |
| **P-18-10** the comment sweep | `1498f46` | Comments only, verified by filtering the patch. Eight platform-doc citations gone (and one platform decision number); the 26 over-64 lines wrapped, leaving only upstream's own; `input.lua` 177 comment lines → **101**, the file 306 → **232**. `help.lua`'s false *"Key has no answer"* reason rewritten with the code left as ruled; `main.lua` names the consequence of the crash path |
...
| **P-18-21** the narrow review | platform `534bd174`; code `e568961` | **Verdict: the batch discharges the third pass's findings; the parity closure is sound; no defects.** Its own measurements went further than the claim they checked — the upstream copy verified byte-identical to `025e858:input.lua`, an upstream **repeat** driver written (the previous pass reasoned that half), and a **385-stimulus** sweep including right-hand and mixed l/r modifier holds the fixed harness never exercised. Zero differences throughout; the pre-fix head reproduces exactly the three named rows. Its one substantive finding is landed: the bare-Alt swallow is **named rather than polled** (`e568961`), so it no longer rests on the device reporting Alt down inside Alt's own `keypressed` — unmeasurable here, which is the argument for not resting on it. Three counts of mine corrected with it (108 not 105; eighteen `[new]` rows not thirteen; the worker's report quoting a comment the parent had moved) |

**The step is now closed to this container.** Every finding from four independent passes is
dispositioned, the gesture-parity diff against upstream is **zero**, and the suite has not moved.
What remains cannot be settled here: **eighteen `[new]` smoke rows, and nothing in this work has ever
run in a game scene, at any head, by anyone.**
