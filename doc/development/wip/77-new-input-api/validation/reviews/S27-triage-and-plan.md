...
| **P14a–e** **[S33]** | **The dissolution itself** — docs/spec → tests → platform code → examples, enumerated in §11.4. **[S33] P14b is RULED** (matcher shape (b), owner 2026-08-09), so a–e are all unblocked. The mock's variadic fix lands first, inside P14c, as its own commit | P8's walk before P14c | §11.4 holds the contents; §11.5 holds the ruling and its costs. Ordering deliberately reverses this table's rule — see §11.2 |
