...
| **P16** **[S40] P16-01 DONE; P16-02 READY** | **P16-01**: `paint` registers its mouse and keyboard handlers as `compy.input.hooks.*`, matching its click handlers; the drag’s `love.mouse.isDown(btn)` poll stays because it answers continuous mouse state. **P16-02**: owner ruling received; remove turtle’s redundant Ctrl+Escape press binding, while retaining its captured callbacks as the example of that path. `balloons` remains deferred. | The common sweep — the in-repo examples and `balloons`; its original input remains `technical_debt/input.md`, §"Examples are not onboarded onto the new input API". | P16-02: platform suite and headless turtle smoke; then independent cold review. |
