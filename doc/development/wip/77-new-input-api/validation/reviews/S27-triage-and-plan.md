...
| P10 **[S43] PART DONE, 2026-08-16** — the **reserved-combo section is written** (`doc/input_api.md`, "Combos the framework keeps"): the two rules (not overridable, never consumes), exactness with the practical corollary that adding a modifier gets you a nearby chord, the nine reservations with when each applies, the play-mode narrowing, and `f10` named as the one bare-key reservation. **W10 batch 1 done** (`9254b524`, `../outcomes/S43-P-10-05-overlay-retirement.md`): 96 of 130 "overlay" sites retired, 34 left with a stated reason each, four headings renamed with their citations fixed in the same commit. **Still owed:** the flag-shortcut teaching defect, W9(a) ledger prune, W9(b) accuracy items, W10 batch 4 (vocabulary). **[S43] ADDED TO THIS ROW:** the reserved-combo section it owes must state **what a reservation claims** — that it does not extend to chords it does not name (Decision 33) — not only which combos are reserved. **[S36] PART DONE, 2026-08-11 — the flag-shortcut member below is DISCHARGED**: §"Shortcuts that set a flag" was **replaced** by §"Choosing the mechanism" (`563b937f`), which states which mechanism answers which question, shows the mirrored pair as a DON'T, and carries Decision 32's no-reconstruction rule; six citations of the old heading were repaired, one of them in the register where it had recommended the antipattern. **Decision 32 landed** (`81e97bab`) with **Decision 30 point 3 amended in place**, and the adoption checklist went to `conventions/input_adoption.md` (`82979e55`). **[S36] Two factual defects found by the disposition pass, neither previously flagged by any marker:** the ledger called the `before_submit` veto *"(still-reserved, unbuilt)"* when it is built and documented (`userInputController.lua:405`, guide §"Submit lifecycle") — **corrected 2026-08-11**; and `internals/event_dispatch_layers.md`'s pointer/keyboard asymmetry passage is reported stale against the register's own *RESOLVED 2026-08-03* entry — **claimed, not yet verified by me, verify before editing**. **What this row still owes:** the reserved-combo section the guide has never had (the P15 row enumerates the set), W9's ledger work, W10 batches 1/2/4, and **the marker question of §16.2, which is this row's real size** — 8 markers in the project guide, 30 in the ledger, 33 in the internals guide. **[S36] NEW MEMBER, and it is a defect in a teaching passage, not a wording one: `doc/input_api.md` §"Shortcuts that set a flag" teaches a pattern whose clearing event can be missed.** Its example binds bare `'space'` on `keypressed` and `keyreleased`; press Space, then press Ctrl, then release Space, and the release serialises as `'ctrl+space'`, so the clearing binding never fires and the flag stays set. The same failure with a modifier in the combo (`'alt+h'`) is **unfixable by any second binding**, since a modifier's own release has no expressible combo (Decision 21). **The section needs the boundary stated — a combo serves an atomic transition, not a held state** — and its example needs either a class-key clearing path or an honest note that the flag is armed by an event and must be cleared by something that cannot be missed. Reasoning and the sketched general mechanism are in `technical_debt/input.md`, "A chord that gates a state while it is held has no vocabulary". Found in session36 while answering why the keyboard example's help overlay still polls **[S38] NEW MEMBER, from the P-18 cold pass: `Key.is_mod` and `Key.is_alt` are exported (`src/util/key.lua`) and appear NOWHERE in `doc/input_api.md`** — "Held keys" documents only `shift`/`ctrl`/`alt`/`any_pressed`. The keyboard example depends on both: `is_mod` in six files, `is_alt` in `input.lua`'s bare-Alt guard. A project cannot be asked to use a member the guide does not have, and the example's PR will show it using two. **[S39] Make that THREE: `Key.is_shift` is in the same position** — exported, undocumented, and now used by `maze`'s macro recorder (`P-17-09`), which asks it whether a pressed key is a Shift key. Two example PRs will now show the guide's readers members the guide does not have. | W9 + W10 (1,2,4) — docs, ledger, vocabulary. **[S33] REDUCED — its Decision-30 slice is pulled forward into P14a** (the project-facing "Held keys" rewrite, the flag-shortcut teaching, the `:268` false claim, the debt register, the Decision 21 tombstone). **What remains here:** the W9 ledger work and W10 batches 1 (retire "overlay"), 2 (no historical contrast) and 4 (vocabulary). **[S33] New members found by session32:** `doc/input_api.md` §"Held keys" needs **replacing, not purging**; Decision 21's worked example is stale — it says a hook "receives the held-key view", which Decision 26 already removed. **[S35] One more member, found while commissioning P15: the project-facing guide never says which combos the framework has already taken.** It teaches `shortcuts.keypressed['ctrl+s'] = ...` as an example while the gateway itself claims `ctrl+s`, and claims `f10` — a **bare, unmodified key** — whenever profiling is on. A project author reading only this guide cannot know which of their bindings will never fire. The reserved set is enumerated in the **P15** row; this step gives it a named section in the guide | P2–P9, **[S33] P14a–e** | docs describe the final code, so they come after it. **Tombstone decisions, never renumber**. **[S29]** Decision 29 and the `input_api.md` "Held keys" rewrite landed early by owner instruction — do not redo them |
| P11 | **[S36] THE GATE'S OWN PATTERN IS INCOMPLETE — found 2026-08-11 and fixed at once.** Two markers spelled `REMARK` **without the colon** (`internals/event_dispatch_layers.md`, `internals/user_input.md`), so `grep -rn 'INTERIM:\|REMARK:'` returned clean while they stood. Both were normalised to carry the colon — punctuation only, their text untouched — and **the gate must be run as `grep -rniE 'INTERIM|REMARK'` at least once before it is trusted**, since a gate that cannot see a marker is worse than no gate. Doc-corpus count therefore **86, not 84**. **[S36] MEASURED, 2026-08-11: 27 markers in `src/`+`tests/` and 86 in the persistent doc corpus (113 total).** The gate as written demands zero; **§16.2 puts three readings of that gate to the owner and recommends one** — clear code and the project guide entirely, clear everything factual in the dev-facing docs, and defer the purely editorial remainder as a **named list** rather than as silence. **[S44] NOT GATED — that question was ruled in session36 and this row was never updated; §16.2 carries the resolution (reading (c) with (b)'s floor) and `../outcomes/S36-marker-disposition.md` is the binding table. The row's size is measured, not unknown: see §17.5.** — W12 — comment sweep, slices, revalidation ×2. **[S39] THIS ROW NOW OWNS THE EXAMPLE REPOS' COMMENT COMPACTION TOO** (owner, 2026-08-13, removing `P-17-13` from the maze step). Compaction runs **after the code stops moving**, and an example's code is not final while its human smoke pass can still send it back — so it cannot live inside the step that wrote the code. **Outstanding: `maze` + `draw`** (`P-17-06`…`P-17-14` wrote deliberately full comments under the 2026-08-13 verbosity ruling, so there is real work here). **`keyboard`'s was already done in-step as `P-18-10`** — 177 comment lines to 101 without losing an argument — which is the model to copy, not a precedent for doing it early. `balloons` and the tracked examples are in scope for the same pass. The marker gate (`grep -rniE 'INTERIM|REMARK'` over `src/` and `tests/`) already reaches the nested repos, since they live under `src/`. **[S33] W10 batch 3 (comment bloat, ~50 ids) belongs here, not P10** — deliberately deferred to a late pass so comments are cut after the code stops moving. **That subset is never separately enumerated inside W10's block of 92 and must be re-derived first** | P10, **[S33] P14a–e** | the commission's (e)–(9). **[S33] the gate is currently FAILING: `grep -rn 'INTERIM:\|REMARK:' src/ tests/` → 22 in the platform and 5 in `src/examples/` (disjoint sets, 27 total; zero `INTERIM:`). It must return nothing before slice regeneration.** **[S33] The gate also covers `PENDING` markers** — including in `doc/`, which the marker sweep has never had to scan before, because P14a puts them there deliberately |
...
| ~~**P16**~~ **DONE [S40]** | **P16-01**: `paint` registers its mouse and keyboard handlers as `compy.input.hooks.*`, matching its click handlers; the drag’s `love.mouse.isDown(btn)` poll stays because it answers continuous mouse state. **P16-02**: the owner ruled turtle’s Ctrl+Escape press binding redundant; the framework owns the release-side quit and does not consume the event. Turtle retains its captured callbacks as the example of that path. `balloons` remains deferred. | The common sweep — the in-repo examples and `balloons`; its original input remains `technical_debt/input.md`, §"Examples are not onboarded onto the new input API". | Platform suite and headless smoke for touched examples; independent cold review before moving on. |
| ~~**P19**~~ **DONE [S41]** | Sapper retains its modifier-held touch fallback. The rare delayed derived-click echo is documented without a project-side guard or platform change. **Operative detail: §15.2b** | P14e | Ledger entry and one-line source constraint comment |
...
| ~~**P-13-01**~~ **DONE [S43]** (`3befd556`) — `git diff 5b580661^ -- src/harmony/` empty, **and `git diff 3256aac -- src/harmony/` is empty too**: the subsystem is byte-identical to the PR base, so this feature now contributes nothing to aldum's code. **Standing preference (owner, 2026-08-16): do not refactor function sizes inside another author's subsystem.** `love_key` is 23 lines at the PR base and 25 at harmony's first commit — we did not bloat it, so no split is owed; `b31e99a9`'s split is correctly gone and must not return | **Revert the harmony change.** `src/harmony/init.lua` and the scenario `release_keys()` calls back to `5b580661^`. That tree is *already correct* under the device-read matcher (§11.3: "Harmony can now drive the combo mechanism it previously could not"), so the revert alone closes the regression — no re-implementation | P-13-00 | **Before the PR** — harmony is outside `busted` and outside CI, so nothing else signals this. Production fix, its own commit, suite green |
| ~~**P-13-02**~~ **DONE [S43]** (`3befd556`) — three cases: push queues and only the drain reaches the app; only the trigger key enters the event stream; the modifier stays held until `release_keys()`. `setup_harmony` split into `mock_state`/`queue_recorder`/`setup_harmony`, all within the limit. Suite 947 → **949** (1 case → 3) | **[S43] RULED** (owner, 2026-08-16) — **keep the spec; rewrite it to match desired behaviour** | `tests/harmony_input_spec.lua` stays, re-aimed at the contract the revert restores: `love.event.push` **queues**, the drain happens after the scenario step, and the simulated modifier is **still held** when the app handles the key — so the device-read matcher fires Ctrl+T, and only an explicit release clears it. The owner frames this as **a late instance of canonicalizing de-facto behaviour** — the same move this feature opened with. Harmony's press/hold/release contract was real but unwritten, and its absence is exactly what let P13's synchronous-push fixture pass as proof. The starting shape is the probe pair in `../notes/S43-harmony-probes/` | **blocks P-13-01** — the spec asserts the P13 behaviour (`Key.ctrl()` falsy after the chord), so it goes red the moment the revert lands; both must be green at the same commit boundary | **Owes a justification-table line** (§11.7), as P15 did: the PR keeps a suite the 187-remark mandate did not ask for. The answer is short — it pins a contract the platform can otherwise break silently, which it just did. The 23-line `setup_harmony` S2 finding is resolved here, by rewrite, not separately |
| ~~**P-13-03**~~ **RULED AND DONE [S43]** (owner, 2026-08-16) — **no emission; nothing to implement.** Combo checking depends on the device poll and harmony plumbs it, so the pre-P13 mechanism is sufficient. What lands instead is a seven-line comment at harmony's `held` table: scripted modifiers never enter the event stream, which is fine while Decision 30 stands, but a consumer that counts events to mirror held keys would see no modifier and misjudge silently. **No debt entry** — there is no such code today, and an entry that blamed harmony for a risk nobody has taken would be noise; if one is ever wanted it must read as a parity-limitation warning, not a defect | **Re-add modifier-event emission, if the owner wants it at all.** Evidence in §10 says it is not needed (Decision 30 = device-only) and that it changes what project hooks observe. If wanted: **additive only** — press events emitted, `held` still cleared by an explicit release, so the poll keeps answering across frames | P-13-01 | **Owner ruling pending.** Default is to skip; reopen only if the batch-skew rig is ever commissioned |
| ~~**P-13-04**~~ **DONE [S43]** — **sound, no findings at any severity** (`../outcomes/S43-P-13-04-cold-review.md`). It traced each of the three cases and confirmed every one fails if the defect returns, and went past its brief on the `Key.ctrl()` zero-return, finding six production call sites where the value is a last argument and proving none is affected | **Cold agent review of the P-13 execution** (owner directive, 2026-08-16 — every execution step gets one). Read `../notes/S43-harmony-p13-timing-finding.md` and the P-13 rows, then check the landed revert and the rewritten spec: is the restored contract the pre-P13 one, does the new fixture model queue-and-drain rather than dispatch-on-push, does the suite arithmetic reconcile, and is anything left of P13 in the tree | P-13-01, P-13-02 | cold agent, prompt of record on disk, deliverable under `../outcomes/`; parent evaluates rather than relays |
...
| ~~**P-20-00**~~ **DONE [S43]** (`837b1313`) | **[S43] P-20 — revalidate the untrailered sessions** (owner, 2026-08-16), opened after session42's P13 defect. **-00 is the authorship audit and this plan.** Method and boundary: `../notes/S43-agent-authorship-audit.md`. Attribution rests on the `Co-Authored-By: Claude` trailer, absent from every commit after `a1842a2f`. **Confidence is not uniform** — sessions 39-tail/40/41/42 also carry a positive foreign fingerprint (literal `\n` escapes in commit bodies, 8 commits); **session38 carries none**, so for it the only evidence is the missing trailer, and the owner recalls it as Claude-run and pedantic. Rows below are numbered by session; the **attention order is P-20-02 first** | — | audit on disk before any review is commissioned |
| ~~**P-20-01**~~ **DONE [S43]** — **session38 stands**; evaluation `../reviews/S43-P-20-01-evaluation.md`, worker report `../outcomes/S43-P-20-01-session38-outcome-review.md`. Two S2s, neither a code defect: the last behavioural commit `e568961` was never read by an independent pass though `84b6e0c5` calls it "the reviewed head", and **the parity harness lived only in `/tmp`** — now preserved and runnable at `../notes/S38-parity-harness/`, re-run at 108 stimuli zero diff | **session38 — review the outcome, not the work.** ~20 commits, all in the nested `keyboard` repo (no platform code). The claim to test is the closing one: gesture behaviour **provably identical to upstream**, *"measured, not argued"*, via a parity harness the session built. **Re-test the harness, not the parity table** — session42's defect was exactly a harness modelling a mechanism the system does not have (synchronous `push`), and a parity harness with the same fidelity gap carries its conclusions with it. Also: did the last batch (`P-18-19…21`) get the same independent review as the first three, or was the last step shortcut? | P-20-00 | outcome-level; escalate to full revalidation only if the harness is found unsound |
| ~~**P-20-02**~~ **DONE [S43]** — **tail sound as far as it goes, but it does not close what it believes it closed.** Evaluation `../reviews/S43-P-20-02-evaluation.md`, worker report `../outcomes/S43-P-20-02-session39-revalidation.md`. **S2, owner ruling needed:** the platform reserves Ctrl+Escape on **release** at the raw pump entry (`controller.lua:882-890` → `:653-684`), so `ctrl+shift+escape` and `ctrl+alt+shift+escape` fire the game's menu on press and then have the project torn down on release — the restoration is cosmetic for two of the four variants, and this is P15's ruled property biting a real project. Probe: `../notes/S43-ctrl-shift-escape-probe.lua`. Nothing had reviewed the tail before this | **session39 tail — full revalidation. THE PRIORITY.** The session was Claude-run through `a1842a2f` and finished by another agent: `faedac15`, `c3b74959`, `56c0c26f`, `230cb32e` (P-17-16), `f45a2588` (track), `5b6eebc0` (wrap + report), plus `da9d1c2` in the nested `maze` repo. The handover fell **mid-step**, between commissioning a cold review and acting on its finding — the worst place for it. Verify the Shift+Escape modifier family against the cold review's actual finding and the owner's ruling, and whether the wrap's claims (42/0/0 nested, 946/0/0/10 platform, "code-complete") hold | P-20-00 | full revalidation per `agents/rules/revalidation.md`; player-visible behaviour, so no claim is accepted on the report's word |
| ~~**P-20-03**~~ **DONE [S43]** — **clean, no findings** (`../outcomes/S43-P-20-03-sessions40-41-sweep.md`) | **sessions 40 and 41 — a cheap sweep.** Small footprints: `src/examples/{paint,turtle}/main.lua` (P16) and one comment in `src/examples/sapper/main.lua` (P19). Confirm the landed code matches what the reports and plan rows claim, and that P16's cold review was real. Session42 is **excluded** — its defect is established and its correction is P-13-01…03 | P-20-00 | lowest priority; may be folded into P-11-01's example pass if it finds nothing |
| ~~**P-20-04**~~ **RULED [S43], 2026-08-16 — option A, all framework cases; carried into P-21** | **The gate reserves tolerantly; projects must register exactly.** Surfaced by P-20-02: maze's `ctrl+shift+escape` fires the game's menu on press and is then torn down on release, because the release gate reads `Key.ctrl() and k == 'escape'` and ignores the rest. Same for `ctrl+alt+shift+escape`; `shift+escape` and `alt+shift+escape` are unaffected. **Pre-existing** — the gate has this shape at PR base `3256aac` — and systemic: `quickswitch` ignores Shift, `f10` names no modifier, and **Ctrl+Alt+Shift+R fires `restart` and `reset` both** (probe verified). Recorded in `technical_debt/input.md`, "The gate reserves tolerantly; projects must register exactly". **Three options, in `../reviews/S43-reserved-combo-tolerance.md`** | P-20-02 | **Owner-gated: any option that touches `controller.lua` is a framework behaviour change and owes a justification-table line (§11.7).** Option C (document only) needs no ruling and is already done |
| ~~**P-21-00**~~ **DONE [S43]** | **[S43] P-21 — framework reservations become exact** (owner ruling, 2026-08-16), ratified as **Decision 33** in `../../../decisions/input.md`: a reservation matches its modifier set exactly. Two reasons, the owner's: a project's richer combo must not **dissolve** into a framework one, and — the stronger — framework shortcuts are **not overridable**, so unlimited power must be granted on the narrowest condition (least privilege). **-00 is the blast radius**: nine reservations tabulated with their exact forms, regressions, and the substeps — `../reviews/S43-P-21-00-blast-radius.md` | P-20-04 | analysis only; no code touched |
| ~~**P-21-01**~~ **DONE [S43]** (`b20a4c35`) — `only_mods(ctrl, alt, shift)` at `controller.lua:388-403`, applied to the release gate; suite 949 → 951 | **The predicate + the release gate** (row 9, `controller.lua:884-885`) — the case that motivated the decision. Converts the `ctrl+escape` pending outline (`tests/input/input_global_shortcuts_spec.lua:95`) into a live pair | P-21-00 | breaking test first; suite green at the commit |
| ~~**P-21-02**~~ **DONE [S43]** (`77aed369`) — rows 1–6 and 8; row 4 excludes Alt only with the reason in a comment, row 7 untouched, the Ctrl+Alt+Shift+R double-fire closed with a case that failed before; suite 951 → **964 / 0 / 0 / 10**, pendings untouched at 10 | **The `keypressed` gate** — rows 1–6 and 8 (quickswitch, suspend, quit, stop/finish, reset, restart, F10). Row 7 (profiler) is **already exact** and must not be "fixed". Includes a live case for the **Ctrl+Alt+Shift+R double-fire**, a real defect this closes | P-21-01 (same handler block — sequenced, not parallel) | the defect fix states its evidence in the message |
| ~~**P-21-05**~~ **DONE [S43]** — **sound**; two findings, both verified by me and both real: (a) S2, nothing anywhere pressed `ctrl+shift+s`, so row 4's Alt-only exclusion was untested against Shift creeping in; (b) S1 by its own scale, the execution report's size/nesting numbers for `project_state_change` were wrong in both directions. Neither is a defect in the sweep. `../outcomes/S43-P-21-05-cold-review.md` | **Cold review of the sweep** — prompt of record `../prompts/S43-P-21-05-cold-review.md`. Runs **before** P-21-03 and P-21-04, since both depend on the sweep being sound. Numbered after them only because they were drafted first | P-21-02 | cold agent, read-only; parent evaluates rather than relays |
| ~~**P-21-06**~~ **DONE [S43]** (`2b89bba8`) — two cases pin Shift as meaningful on `ctrl+s`, proved by temporarily adding `not Key.shift()` and watching only the shift case fail; the report's numbers corrected in place with a dated marker. Suite **966 / 0 / 0 / 10** | **Close the two P-21-05 findings** — add the missing `ctrl+shift+s` coverage (proved by deliberately breaking row 4 first), and correct the numbers in the execution report in place, marked as amended. Prompt of record `../prompts/S43-P-21-06-review-fixes.md` | P-21-05 | production code is **not** to be touched; the sweep was reviewed clean |
| ~~**P-21-03**~~ **DONE [S43]** (`c23cb59` in the nested repo) — note removed, specs 42/0/0, and the **original defect probe now passes**: shortcut fires 1, platform asks to quit 0, `stop_project_run` 0 | **Remove maze/draw's risk note** (`7947ac3` in the nested repo) and confirm the four-variant Shift+Escape family end to end; the note's stated removal condition is met once P-21-02 lands | P-21-02 | nested repo commit, never pushed |
| **P-21-04** **PART DONE [S43]** — debt entry closed against Decision 33 (kept, marked RESOLVED, with the route-level half named as still open); PR paragraphs drafted in `../notes/S43-pr-lines-owed.md` for the regeneration step. **Still owed: the guide's reserved-combo section, which is P10's row** and must now state that a reservation does not extend to chords it does not name | **Docs and the PR line.** P10's reserved-combo section states what a reservation claims; the debt entry "The gate reserves tolerantly; projects must register exactly" closes, pointing at Decision 33; the justification line is drafted — this is a framework behaviour change inside an input-API mandate and a stakeholder will ask | P-21-03 | last, after behaviour settles |
| ~~**P-22-01**~~ **DONE [S43]** (`4e93ff78` patch + test, `802ff1a4` comment) — one line: the locked, not-held path returns `false`. `git diff 3256aac -- src/harmony/` is now +8 lines total, all of them this feature's two deliberate exceptions. Suite **967 / 0 / 0 / 10** | **[S43] Harmony's `isDown` patch answers a boolean** (owner ruling, 2026-08-16). Rationale is signature parity — a mock matches the thing it mocks; `love.keyboard.isDown` returns a boolean and the locked, not-held path returned no value at all. A deliberate, surgical exception to the standing rule against touching aldum's subsystem. Prompt `../prompts/S43-P-22-01-harmony-isdown-boolean.md` | — | smallest edit achieving parity, plus one case that fails if the value regresses to nothing, plus restating `only_mods`'s now-stale comment. Debt entry "A modifier accessor answers truthy/falsy, not a boolean" narrows to `Key`'s own unenforced annotation |
| ~~**P-23-00a**~~ **DONE [S43]** (committed as P-23-00; relettered when -00b was added) | **[S43] Ctrl+S leaves the pre-dispatch gate** (owner proposal, 2026-08-16): the stop-a-run meaning belongs to `ProjectInputController`, enforced before the chain walk; the editor's two meanings belong to `EditorController`. **Feasibility: yes, and small** — `occupy_input` installs the project route unconditionally for every `running` state, so the right controller is guaranteed to own the keyboard. Analysis, risks and test obligations: `../reviews/S43-P-23-00-ctrl-s-relocation.md` | P-21 | analysis only; no code touched |
| ~~**P-23-00b**~~ **DONE [S43]** | **[S43] Pen-and-paper projects, and the canvas/error boundary** (owner, 2026-08-16), preceding -01. **Two findings.** (1) A non-blocking project keeps the route in `project_open`, where `ctrl+s` currently belongs to **the project** — so the obvious implementation, checking at the top of PIC's walk, would silently take it away and stop such projects on their own key. The `running` scope is what prevents that, and it needs a live test. (2) The boundary argues against the obvious shape: `with_canvas_and_errors`'s error handler calls **`suspend_run`**, so a raise inside `stop_project_run` would suspend the run mid-teardown and report it as the project's fault — in the one path whose purpose is recovery. Also `use_canvas` does not nest. **Recommendation AMENDED after the owner asked whether this is overengineered — it was.** The deferral channel is withdrawn: it protects a path that only opens once we have shipped a framework bug. Two simple options remain — **A** move only the editor half (no PIC change, no state read, no boundary question), **B** both halves with PIC calling `stop_project_run` directly behind one state read. Owner ruling needed. `../reviews/S43-P-23-00b-liveness-and-boundary.md` | P-23-00a | analysis only; no code touched |
| ~~**P-23-01**~~ **WITHDRAWN [S43], 2026-08-16 — owner chose option A.** The run-stop meaning stays in the gate with its family (Ctrl+Escape, Ctrl+Q, Ctrl+T): all stop or tear down a run, all non-overridable. Moving one member out would split the family and buy a `project_open` scope problem and a boundary problem for it. The `ctrl+s` reservation is instead made **exact** in place, under P-23-02 | **The run-stop half.** A platform-reservation check in `ProjectInputController` ahead of the three-consumer walk, scoped to `app_state == 'running'` and **not** widened to `project_open`. Exclusive: `only_mods(true,false,false)`. Keeps P15's property — a project binding `ctrl+s` still cannot take it | P-23-00 | tests: teardown from inside the route boundary; a project's own `ctrl+s` loses; the P-21 row-4 cases re-pointed, not deleted |
| ~~**P-23-02**~~ **DONE [S43]** (`cb6b867e` the move, `5e6ad8c2` extracting our own seven lines back out of an over-limit function). The P-21-06 editor cases survived **byte-identical and green** — they assert behaviour, not layering. Suite **968 / 0 / 0 / 10** — commissioned as option A: the gate keeps an **exact** run-stop `ctrl+s` (`only_mods(true,false,false)`) and the editor's two meanings move to `EditorController:keypressed`. One intended behaviour change: **Ctrl+Shift+S no longer stops a run**. Prompt `../prompts/S43-P-23-02-editor-half.md` | **The editor half.** `ctrl+s` closes the buffer and `ctrl+shift+s` finishes the edit, in `EditorController:keypressed` beside the existing `ctrl+m`/`ctrl+f` branches; the gate's `k == "s"` block is then deleted whole. Exclusive on both. Shortcut-syntax migration is **not** in scope — noted as the later option the owner named | P-23-01 | one case per meaning, plus Ctrl+S in `ready`/`project_open` still doing nothing |
| ~~**P-23-03**~~ **DONE [S43]** — **faithful move, exactly one intended behaviour change**, verified across `{ctrl+s, ctrl+shift+s, ctrl+alt+s, ctrl+alt+shift+s}` × `{running, project_open, editor, ready, inspect}`; `project_open` untouched, so pen-and-paper projects keep their own `ctrl+s`. Three drift findings, **all fixed in `%s`**: `internals/user_input.md` still said the gate closes buffers (S2); a spec comment claimed the two branches share an outer condition, which exactness made false (S2); Decision 33's example list did not yet name Ctrl+Shift+S (S3). **Separately noted, not ours and not fixed:** `internals/project_sandbox_env.md:81` and neighbours cite `controller.lua` line numbers that were already stale before this session — line-number citations, which the conventions discourage in favour of named sections | **Cold agent review** of P-23-02/-02b, per the standing directive | P-23-02 | read-only; parent evaluates |
| ~~**P-24-00**~~ **DONE [S43]** | **[S43] `only_mods`'s positional API** (owner, 2026-08-16). Reach: **seven call sites, all file-local in `controller.lua`, one day old** — no test, doc or module names it, so it is as cheap to change as it will ever be. Three options weighed in `../reviews/S43-P-24-00-only-mods-api.md`; **recommendation is to delete the predicate** and compare the canonical combo string the project already builds (`combo_string(k)` → `'ctrl+shift+s'`, verified against live code), which makes exactness structural rather than a rule to remember, puts the gate in the same vocabulary a project writes (Decision 27), and drops up to 21 device reads per keypress to 3 | P-23 | analysis only; no code touched |
| ~~**P-24-00b**~~ **DONE [S43]** | **[S43] The owner's three follow-ups** (2026-08-16): use the shortcuts paradigm instead of if-then; reuse a fixed slot table instead of allocating; share the assembled combo between gate and route. Answers, with evidence: `../reviews/S43-P-24-00b-table-and-sharing.md`. **The table is the thing Decision 30 point 3 explicitly declined** — reversible, but by amending a ruling, not by slipping a refactor in; **sharing is recommended against** on this feature's own ground, since a per-event combo cache is the `keys_pressed` shape Decision 30 dissolved, and `_dispatch` is reachable without the gate | P-24-00 | analysis only |
| ~~**P-24-01**~~ **DONE [S43]** (`737d8316`) — string accumulation, no table and no buffer. Debt entry closed. `find_shortcut`'s double build stays: reusing the first walk needs a parameter, cached state, or a second copy of Decision 8's precedence, all worse than the cost | **Remove the allocation from `combo_string`** — no table at all rather than a reused buffer, which kills the reentrancy hazard with it. Note it is up to **two** allocations per event today, not one (`find_shortcut` builds the exact combo then the `'*'` class) | P-24-00b | surgical; existing suite is the proof, no test may need editing |
| ~~**P-24-02**~~ **DONE [S43]** (`f31bd312`) — `RESERVED.keypressed` / `.keyreleased`, one named function per reservation, never-consumes contract stated at the table. Gate device reads **23 → 3** per event; route unchanged at 3 (6 on a `'*'` fallthrough). **No test edited**; suite 968 / 0 / 0 / 10. **[S43] RULED, 2026-08-16.** The owner's reading corrected mine: Decision 30 point 3 declines to *commit* to the table and guards against **confusing** the two, it does not prohibit one. Ruled and recorded as **Decision 34**. The separate combo-string-comparison step is folded in — landing a cascade of string comparisons we would immediately replace is churn | **Build the privileged reservation table** — `reserved.keypressed['ctrl+t']`, per event, mirroring the project shape. Decision 33's exactness is the prerequisite that makes one entry per chord well-defined. **The non-consumption contract must be stated at the table**: a project's shortcut consumes by returning truthy, a reservation never consumes — same shape, opposite rule. Subsumes the if-then question; also makes the reserved set listable, which is what P10's guide section owes | P-24-01 | medium; own cold review, and the Decision 33 cases are the equivalence proof — no test may need editing |
| ~~**P-24-03**~~ **DONE [S43]** | **Cold review of the reservation table — representation-only, no findings at any severity.** It partitioned the playback-guarded set against the old dev-only set and found them identical, confirmed the `f10` no-`else` property survived the lookup, and checked both handlers still forward unconditionally. Unprompted, it settled the rebuild question: `setup_callback_handlers` runs **once at boot** (`main.lua:385`), whereas the old reservation closures were declared inside `handlers.keypressed` and reallocated **per keypress** — so the table allocates strictly less than the cascade did. `../outcomes/S43-P-24-03-cold-review.md` | P-24-02 | — |
| **P-11-01** | **Embedded-example input-shape review.** Review every embedded example for balanced use of `compy.input.hooks` versus captured `love.callbacks`, and direct polling used only for continuous device state. Record each retention, migration candidate, or deliberate exception before comment compaction. | P10 and all example code/smoke work | Read-only; findings feed P11 and separately planned follow-up |
...
### [S43] That result is WRONG — P13 is reverted by P13a

The owner contested their own ruling on 2026-08-16 and was right. The
"revalidation" above confirmed nothing: its fixture replaces `love.event.push`
with a synchronous dispatcher, which is the one arrangement where a released
modifier still reads as held. Under the real loop the modifier is cleared a
frame before the app sees the key, and **every scripted chord is delivered
bare**. Full finding, mechanism and A/B reproduction:
`../notes/S43-harmony-p13-timing-finding.md`.

Note the 2026-08-09 ruling was **conditional** — retire the discipline *if that
confirms*. The condition was never met, so the retirement was never ratified.

### [S43] Is modifier-event emission wanted at all? — evidence for the owner

The question is the owner's (2026-08-16) and the ruling is theirs. What the code
says:

- **Not needed.** Decision 30 makes modifier state device-only (`keys_pressed`
  dissolved); `Key.ctrl()`/`shift()`/`alt()` are `love.keyboard.isDown`
  (`src/util/key.lua:156-178`), which is precisely what `patch_isDown` replaces.
  The combo builder reads the device too (Decision 30 point 2). **No framework
  consumer reads a modifier's press *event* for modifier state.** The project
  dispatcher goes further and **drops modifier triggers outright** —
...
- **The one thing it would buy** is fidelity to real hardware, which does emit
  ...
  dropped and which nothing currently asks for.

Reading these together: the defensible default is **revert and stop** — no
emission — and to reopen it only if the batch-skew rig is ever commissioned. If
...
| ~~**P9c**~~ **DONE [S42]** (`6c127229`) | **Unaffected in premise.** But P14c moves and deletes test cases, so **re-check both order-dependent cases after P14c**, not before. **[S33]** same re-lettering correction as the P8 row above |
...
**The owner is the example author.** The owner may decide the behaviour directly; the PR
description still owes a line about the result either way (§11.7).

**[S41] P19 COMPLETE.** The owner retained the touch fallback and accepted the
rare delayed derived-click echo without a project-side guard. `P-19-01` records
that disposition in the global debt ledger and the source comment states the
arrival-time modifier constraint. No platform change is proposed.
...
3. **P17 — maze deepfix:** independent of P18. P19 is complete [S41]: its
   rare delayed-click effect is documented without a new guard.
...
6. **P9c** — the two order-dependent test cases, now that the suite moves have landed.
...
7. **P11 — the comment sweep over `src/` and `tests/`**, which is this sprint's half of the marker
   work: 27 markers, and the gate for them is unambiguous. **[S39] It also owns the example repos'
   comment COMPACTION** — `P-17-13` was removed from the maze step on 2026-08-13 because compaction
   runs after the code stops moving, and an example's code is not final while its smoke pass can
   still send it back. `maze`/`draw` are outstanding; `keyboard`'s was done in-step (`P-18-10`) and
   is the model.

   **P-11-01 comes first:** review all embedded examples for balanced use of
   hooks, captured `love.callbacks`, and direct polling. Record the disposition
   before compacting comments; this creates no migration by implication.
...
### 17.5 [S44] P11 is not gated, and this is its size

Two pieces of framing carried into session44's prompt were stale. Both are
retired here so the next session plans against facts.

**(a) The §16.2 marker question is not open.** The P11 row said the row's size
was unknown "until that is ruled". It was ruled in **session36**: §16.2 records
**RESOLVED IN PRACTICE, 2026-08-11** — reading **(c) with (b)'s floor** — and
`../outcomes/S36-marker-disposition.md` is the binding table that bound every
marker to a plan. The row simply never absorbed it. Restated so it is not
re-litigated:

- **`src/`, `tests/` and `doc/input_api.md`: every marker goes.** The code, and
  the one document a stakeholder is promised.
- **Dev-facing docs (`internals/`, `decisions/`, the registers): everything
  factually wrong or misleading goes.** A false statement shipping is a defect.
- **The purely editorial remainder is deferred as a NAMED LIST**, not as
  silence — the PR description carries the list.

**(b) "P11 runs cold with the owner's own planning changes" is not a
commitment anyone can act on.** It traces to a single line in session43's track
and nothing was ever written behind it; the owner does not recall a specific
change and does not treat it as blocking (2026-08-25). **P11 therefore waits on
nobody.** Running it in a *fresh* session is still the recommendation — it is a
large block and this one is long and heterogeneous — but that is session
hygiene, not a gate.

**Census, measured 2026-08-25** (the last count was 2026-08-11 and the tree has
moved since; markers were retired by sessions 43 and 44 as the work answering
them landed):

| Where | Markers | Disposition |
|---|---|---|
| `src/` + `tests/` | **22** (1 of them in `src/examples/`) | all clear |
| `doc/input_api.md` | **8** | all clear |
| `internals/user_input.md` | **33** | factual clear, editorial → named list |
| `decisions/input.md` | **24** | same |
| `tests.md`, `event_dispatch_layers.md`, `technical_debt/general.md` | **5** | same |
| **Total** | **100** | was 113 at the session36 count |

**Also in the row, and not counted above:** W10 batch 3 (comment bloat, ~50
remark ids) — **never separately enumerated, must be re-derived first** — and
the `maze` + `draw` comment compaction in the nested repos. `keyboard`'s was
done in-step (`P-18-10`, 177 comment lines → 101) and is the model.

**One caution for whoever runs it** (raised by the owner, 2026-08-25: does
compaction undermine future rework by removing what helps a reader — human or
assistant — understand relations and decisions?). The answer is that
`agents/rules/commenting.md` already protects the payloads that carry
understanding (intent/constraint not obvious from the code; non-local context
pointing at a decision or contract), and compaction removes history,
intermediate rulings, second phrasings and restatement of the code. The load
that matters therefore rests on **citations resolving into the persistent
corpus**. The practical rule, and it is cheap: **any rationale a compaction
removes must already exist in the persistent corpus, or it lands there first.**
