...
- **A withdrawn contract.** `compy.input.keys_pressed` was a **documented project-facing API**, so
  ...
  view, the read-from-outside-a-handler case and the write-raises guard. Deleting them **is** the
  dissolution — the contract no longer exists, so neither does its spec.
...
#### 11.4.2 [S35] P14e in full — the examples reconciliation (OPERATIVE, factored out of the row above)
...
**The leads themselves are operative and live in §11.4.2**, with the examples step. This section is
...
modifier test inside an event handler is a combo by hand. Both are enumerated in §11.4.2, together
