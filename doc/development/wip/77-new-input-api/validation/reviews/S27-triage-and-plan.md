...
| **P-24-02** | **Replace `only_mods` with canonical combo-string comparison at the gate** | P-24-01 | surgical; same proof rule |
| **P-24-03** **OWNER-GATED** | **Amend Decision 30 point 3 and build the privileged reservation table** — `reserved.keypressed['ctrl+t']`, per event, mirroring the project shape. Decision 33's exactness is the prerequisite that makes one entry per chord well-defined. **The non-consumption contract must be stated at the table**: a project's shortcut consumes by returning truthy, a reservation never consumes — same shape, opposite rule. Subsumes the if-then question; also makes the reserved set listable, which is what P10's guide section owes | P-24-02 | medium, not surgical; own cold review |
