# design — version 01 (pre-E29 corpus) — FROZEN SNAPSHOT
...
> frozen content** of `design.md` as it stood before the E29 re-derivation. **This is the
> corpus that carried the day-one derivation mutation** (§4: "`on_key_pressed`'s default value
> *is* the sink; replacing removes it" — replace-semantics), falsified against the architect's
> model at Gate 1 (see [`notes/ratified-model.md`](../notes/ratified-model.md) and
> [`../../notes/talk/m4-pushback-rulings-and-provenance.md`](../../notes/talk/m4-pushback-rulings-and-provenance.md)).
> Kept as salvage material + regression-diff baseline, **not authority**.
