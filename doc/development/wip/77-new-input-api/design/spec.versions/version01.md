# spec — version 01 (pre-E29 corpus) — FROZEN SNAPSHOT
_Generated by LLM(Claude Opus 4.8): 2026-06 (original derivation, commit 207bb01 lineage); frozen as a full snapshot on demotion (E29 Stage 2, session33) 2026-07-05 by LLM(Claude Fable 5); Approved by human?: NO (superseded);_

> **Frozen, snapshot-complete.** Non-latest version per doc-format §6.1 — holds the **full
> frozen content** of `spec.md` as it stood before the E29 re-derivation. Its §3 (channel
> callbacks defaulting to the sink, replace-semantics) inherited the day-one derivation
> mutation removed at Gate 1. Kept as salvage material (round-2 stakeholder records, edge-case
> wording) + regression-diff baseline, **not authority**.

---

# Feature #77 — API Specification
_Generated by LLM(Claude Opus 4.8): 2026-06; per-milestone slices index added 2026-06-16 (E4); Approved by human?: NOT YET;_
...
*(One line per surface; full signatures, defaults, and edge cases are in §1–§7. The
build-order re-cut is the per-milestone slice index below.)*

- **`compy.keys_pressed`** — live `{keyname → true}` set (stays global); downstream as a
  read-only proxy. `combo_string` serialises modifier-first, l/r-folded (`"ctrl+s"`).
- **Singleton lifecycle** — `compy.input.show(config)` / `hide()` / `configure()` / `clear()`.
  `show()` while active is a no-op unless `{force=true}`.
- **Cursor/text** — `compy.input.get_cursor()` / `set_cursor(line,col)` / `set_text(text[,keep_cursor])`
  (2D, 1-based source-line coords).
- **Channel callbacks** — `compy.input.on_key_pressed(k,keys,isrepeat)` and
  `on_text_entered(text,keys)`; their **default value is the respective text-editing sink**
  (assigning replaces it). Both LÖVE channels fire independently (no suppression).
- **Per-combo handlers** — `compy.input.handlers[combo]` (metatable-normalised; return truthy
  to consume the chain).
- **Session hooks** — `before_submit`/`after_submit`, `before_cancel`/`after_cancel`
  (framework owns the middle step; hooks can't suppress it).
- **`on_limit_reached(direction, scope)`** — `up/down/left/right` × `input/line`.
- **Legacy text-input globals removed** (D-1); `ProjectInputController` owns the
  `love.keypressed` slot during project run, with native-handler coexistence (D-9).

> **Two views of this spec.** This file is the **cross-cutting** contract, organised by
> API surface (§1–§7) — read it to understand a whole surface end-to-end. The **per-milestone
> slices** under [`spec/`](spec/) re-cut the same contract by **roadmap build order** — each
> `spec/MN.md` is the self-contained requirement packet a sprint implementing milestone *MN*
> consumes (its scope, the decisions it realises, the exact contract, files touched, and
> acceptance). The slices are **derived from this file + `roadmap.md` + `decisions.md`**; this
> file stays canonical for the contract detail.
...
## Per-milestone slices

Build-order re-cut of this contract (roadmap milestone → slice → primary surfaces here).
M3 was removed (D-1 discarded — see `roadmap.md`); numbering is preserved.

| Milestone | Slice | Primary surfaces (this file) | Realises |
|---|---|---|---|
| M1 — `keys_pressed` table | [`spec/M1.md`](spec/M1.md) | §1 | D-3 (combo form) |
| M2 — singleton extraction | [`spec/M2.md`](spec/M2.md) | §2 (`show`/`hide`), §7 (show-while-active) | D-2 |
| M4 — `ProjectInputController` + gate removal | [`spec/M4.md`](spec/M4.md) | §6 | D-7, D-9 |
| M5 — three-level dispatch | [`spec/M5.md`](spec/M5.md) | §3 (`handlers`/`on_key_pressed`/`on_text_entered`), §1 (registration) | D-3, D-6 |
| M6 — before/after chains + boundary | [`spec/M6.md`](spec/M6.md) | §3 (submit/cancel), §4, §7 (eval failure) | D-4, D-5 |
| M7 — extended singleton API | [`spec/M7.md`](spec/M7.md) | §2 (`configure`/`clear`/cursor/`set_text`) | D-8 |
| M8 — legacy removal + migration | [`spec/M8.md`](spec/M8.md) | §5 | D-1, D-10 |
