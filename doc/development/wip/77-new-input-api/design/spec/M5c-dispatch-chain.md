...
_Generated by LLM(Claude Fable 5): 2026-07-06 (brainlab session33, E29 Stage 3); amended by LLM(Claude Opus 4.8): 2026-07-06 (brainlab session35 — Gate-3 cold-validation retro-reconciliation, human-directed); Approved by human?: NOT YET (Gate 3 in review — frozen on approval);_
...
10. **Suite retro-reconciliation (E29 — the landed suite predates the re-derivation).** The
    ...
    Bucket-B rows** from the AC set below — it does not merely convert the pending rows:
    - `a native handler coexists with the sink` currently green-asserts that showing the widget
      **suppresses** the native handler ("keys go to the sink instead" — the `native_split`
      lifecycle-split wrapper). That is the reversed day-one mutation; under R7/AC-31 it **flips**
      — the native sees events while the widget is shown and falls through to the sink. Re-derive it.
    >> REVIEW: formulate harder. First, let's use world "generic" callback ('native' is legacy meaning e.g. 'love.keypressed' which is wrapped into chain participant. By the terminology of chain we speak of this participant as 'generic' callback (contrary to 'specific' combo-handlers). What we need to test is not only that its invoked regardless of widget state, but also that it can *intercept* processing -- whether the widget will receive the event, depends on what generic callback return. Test also that absence of generic callback set by project is equivalent to non-intercepting (widget receives event).
    - Re-check the sibling green rows (`project keys reach the project sink`, `stop names the
      console as restored route`, `the keypressed path carries the triple`) against AC-1/AC-8/AC-29
      — same model, but re-verified, not assumed.
      >> REVIEW: re-check is a design-level work, not implementor-level?
...
[`../notes/deferred-input-ergonomics.md`](../notes/deferred-input-ergonomics.md)).
...
- **AC-36** The M4-landed green Bucket-B rows are **re-derived from this slice's ACs, not
  preserved as-is**: the native-coexistence row asserts the native handler **receives events
  while the widget is shown** (AC-31) and falls through to the sink on a falsey return — the prior
  suppress-while-shown assertion is gone. No green row anywhere in the suite still encodes
  replace/suppress semantics (R13).
...
Bucket-B rows are re-derived against the ACs (AC-36), not preserved — the native-coexistence
assertion flips (AC-31); the suite's `-- REVIEW:` comments are reconciled (AC-37).** C3 rule: test
