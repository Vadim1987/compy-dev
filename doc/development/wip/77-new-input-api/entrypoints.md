...
| E17 | **Operational — MCP-LSP tooling for the agent fleet** *(proposed, pending go-ahead)*. Stand up the `lua-language-server`-over-MCP wiring (docker-compose + socat blueprint) and smoke-test it from a **read-only codebase-Q&A agent** (the clear, immediate use-case, model: Sonnet 4.6 / Haiku 4.5 for mechanical lookups); only then decide whether to wire the **developer** (Sonnet) + **reviewer** (Opus — highest payoff: find-references/impact analysis). **Not** for the orchestrator (works on docs, not code). **Off the #77 critical path** — non-blocking, recurring/operational class like E10/E11. | ○ | — | `notes/talk/mcp-lsp-tooling.md` |
