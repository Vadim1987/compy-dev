...
| suite | **1025 / 0 / 0 / 10** — 1023 + `BUG-01-09`'s two breaking tests (the multi-line string, at `show` and at the live `set_text`); 1023 was 1021 + `FEAT-02`'s two (the `configure` disarm that keeps the draft, and `false` as the unset; two further cases replaced the ones pinning the retired category); the 10 pending are an owner ruling, an 11th is a finding |
