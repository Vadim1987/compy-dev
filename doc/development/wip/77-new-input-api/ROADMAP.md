...
## ⬜ FEAT-03 — `get_text()`, the read of its own content the surface never had — **in the brace, before `CHG-01`**
...
promoted from an unslugged BACKLOG proposal the same day).
...
that. The advice is not followable today; the guide now discloses the gap, which is the **interim**
state the owner ruled, not the resolution.

**Why it sits in the brace, before `CHG-01`** — `FEAT-01` and `FEAT-02`'s placement argument, at
smaller scale: it changes the **public surface**, so `CHG-01` must carry its line, `ACC-02` should
exercise it on the device, and a slice cut before it lands is cut twice. It is the **third and last**
surface change, and unlike the first two it is an addition rather than a break: nothing that works
today stops working.

| id | step | notes |
|---|---|---|
| FEAT-03-01 | the breaking test first | `agents/development.md`. Content seated by `show` reads back; content the **user typed** reads back (the case the whole entry is about); `nil` while hidden |
| FEAT-03-02 | implement `get_text` on the surface | `consoleController.lua`, `build_widget_api`, beside `get_cursor` and shaped like it: **`nil` while hidden and no warning** — a read of *"nothing to report"* is not a refused mutation. Read-only; it adds no rule about content lifetime, which is the whole reason it was preferred over restoring preservation |
| FEAT-03-03 | `doc/input_api.md` | the surface entry, **and the `hide()` section's disclosure paragraph is rewritten with it** — it says today that no call reads the content, and that stops being true here |
...
**It owes a citation sweep on landing, and the sites are known today.** *"There is no content getter
on `compy.input`"* is asserted as a supporting fact in at least three live places — `D-CFG-BOUNDARY`
(`decisions/input.md`, amended 2026-09-03 to point here), the `set_text` list-branch entry in
`technical_debt/input.md`'s `RETIRED` section, and this roadmap's `BUG-02-01` cell — and each stops
being true when `-02` lands. **A claim spreads by being restated in passing** (`FIX-02-06`'s rule):
sweep by sense, not by the phrase.

**The return shape is the one open question, and it is small.** `set_text` accepts a string or a
list of lines, and `on_text_entered` delivers one string while `after_submit` delivers lines. A
getter that answers a string is symmetrical with `on_text_entered` and loses the line structure; one
that answers lines is symmetrical with `after_submit` and makes the common case (`== ''`) awkward.
Decide it at `-01`, in the test, and state the reason in the guide.
...
notes, ledger entries and live goals (`T-RETIRED-UNVER`, `T-NEVER-SHIPPED`; `T-KEYSET-SPLIT`,
...
| **FIX-02-07** | execute the remaining remark dispositions — **was 37; re-count when the row opens** | triage **complete**; breadth known. Five dispositions were executed early at `FIX-02-03`/`-04`/`-06` (2026-09-02), each because the marker's defect was solved in the same pass (`DEC-02-04`'s rule), and those three cells say so. Markers standing at HEAD: **34 across 12 files** (counted 2026-09-03; markers are not one-to-one with dispositions, so this sizes the row rather than restating its count) |
