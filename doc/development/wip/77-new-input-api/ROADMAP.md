...
*(Struck rows are done, 2026-09-02 session67. `-22` is amber, not green: its frozen-tree half is a
proposal awaiting the owner and nothing else in the half waits on it.)*

- ~~**`-22` is the sharpest case:**~~ **DONE where it is ours, 2026-09-02 (session67), with `-13`.**
  The sites a pass or a reviewer reads are corrected; the frozen `design/` sites are **proposed, not
  edited** — see the row, which also records that the inventory was **five sites and two claims**
  rather than the two-and-one this note assumed, and that *"the code clears it"* meant `show`, not
  `hide`. **The remaining ruling is whether `design/` is amended at all**, and the recommendation on
  the row is that it is not.
