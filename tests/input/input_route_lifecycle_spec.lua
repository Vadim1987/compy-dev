    ...
    describe('stop teardown', function()
      ...
      it('resets the widget\'s own output fields',
        function()
          ...
          F.cc:stop_project_run()
          assert.is_nil(F.singleton.callbacks.validator)
          assert.is_nil(F.singleton.callbacks.on_text_entered)
          assert.equal(noop, F.singleton.callbacks.on_limit_reached)
