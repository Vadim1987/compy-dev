...
describe('Editor #editor', function()
  ...
  describe('structured (lua) works', function()
    ...
    describe('known bugs #regressions', function()
      ...
      -- bug: https://github.com/compy-toys/compy/issues/117#p2
      it('prepends empty line before comment block', function()
        local comment = '-- comment'
        local src = comment .. '\nprint("x")'
        local press, input, saved = open_block_for_edit(src, 1)
        input:set_text({ '', comment })
        mock.keystroke('return', press)

        local expected = { '', comment, 'print("x")', '' }
        assert.same(string.unlines(expected), saved(),
          "empty line before comment, not after")
      end)
    end)
  end)
end)
